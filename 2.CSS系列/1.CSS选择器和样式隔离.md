...
使用方法（webpack默认内嵌了CSS Module）：

```jsx
import style from "./index.less";
```

在需要使用到样式的地方使用 变量名.样式名。 即 `className={style.col999}`

有时为了区分 普通样式表1和 CSS Module样式表 ，我们通常会给 CSS Module 样式表添加别名 xxx.module.less

```js
import "./index.less";  // 全局样式
import style from "./index.module.less";   // 局部样式
```

注意 ：使用 CSS Module 无法修改嵌套组件 的样式（如antd组件的样式或子组件样式），还是需要使用全局样式。

有两种方案：
第一：同时使用 `import "./index.less"`（全局样式）
第二：使用 `:global{ }` 包裹样式，这样使用 CSS Module导入样式的时候 `:global` 包裹的样式还是全局样式，不会被编译添加后缀
注意点：CSS Module 中使用元素选择器 `h3 { color: #999;}` 同样是不会被编译的。
