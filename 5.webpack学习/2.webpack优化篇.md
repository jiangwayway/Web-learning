...
1.treeshaking
treeshaking在webpack流程中的什么时候生效,treeshaking在经过所有的loader处理后才会进行以下阶段，但是我有一个问题，babel-loader会把es6转化为es5，而treeshaking必须要es6模块才能生效，我看网上都说不将es6转化为es5就行了，那es6模块浏览器可以识别运行吗？使用webpack的目的不就是要将es6模块转化为commonjs模块吗？
标记阶段（usedExports）
删除阶段（Terser）
