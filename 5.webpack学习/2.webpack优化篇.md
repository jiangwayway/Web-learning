...
什么是副作用：个人理解，如果一个模块中除了被导出的变量外，还有其他被声明的变量且该变量被使用了。则声明该变量的代码和使用该变量的代码就是副作用代码。

副作用的危害：在我们使用treeshaking的时候，如果导入了某个模块中导入的对象，但是我们未使用该对象，按道理来说该模块应该会被删除，但是如果该模块中含有副作用代码，则该模块依然会被打包且副作用代码会正常执行，之所以会这样是因为在打包时，所有被import的模块都是以立即执行函数的形式存放在`__webpack_exports__`数组中，当在其他模块中导入该模块时，就会通过数组索引找到该模块的立即执行函数并立刻执行，并将导出的变量存放在`__webpack_exports__`中进行返回，在该模块的立即执行函数运行的时候就会运行副作用代码。当使用treeshaking时标记和删除的是导出的变量，如果该模块所有导出的变量都未被使用，则返回的`__webpack_exports__`为空，但这并不会影响副作用代码的运行，导致整个模块还是会被打包进bundle中。
